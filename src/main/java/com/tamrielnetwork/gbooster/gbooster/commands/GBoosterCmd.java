...
public class GBoosterCmd implements CommandExecutor {
	...
	private void executeUse(CommandSender sender, String[] args) {
		...
		for (Booster b : main.getActiveBoostersManager().getActiveBoosters().keySet()) {
			if (b.getBoosterType().equals(booster.getBoosterType())) {
				sender.sendMessage(ChatColor.RED + "There is an active booster of that type already, please wait until it has expired");
				return;
			}
		}
